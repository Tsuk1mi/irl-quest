...
pub async fn websocket_handler(
    ...
    let user = current_user.ok_or(AppError::Unauthorized(
        "WebSocket requires authentication".to_string(),
    ))?;

    tracing::info!("WebSocket upgrade requested by user: {}", user.0.id);

    Ok(ws
        .on_upgrade(move |socket| handle_socket(socket, user.0.id, user.0.username.clone(), state)))
}

/// Обработка WebSocket соединения
async fn handle_socket(socket: WebSocket, user_id: i32, username: String, state: AppState) {
