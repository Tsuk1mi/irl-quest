...
pub async fn setup_mfa(
    ...
    let setup_response = auth_service
        .setup_mfa(&state.db, user.0.id)
        .await?;
...
pub async fn disable_mfa(
    ...
    auth_service
        .disable_mfa(&state.db, user.0.id)
        .await?;
...
pub async fn oauth_login(
    ...
    let oauth_info = match request.provider {
        OAuthProvider::Google => {
            let redirect_uri = request.redirect_uri.as_deref().unwrap_or("http://localhost:8003/oauth/callback");
            oauth_service.authenticate_google(&state.db, &request.code, redirect_uri).await?
        },
        OAuthProvider::Apple => {
            let redirect_uri = request.redirect_uri.as_deref().unwrap_or("http://localhost:8003/oauth/callback");
            oauth_service.authenticate_apple(&state.db, &request.code, redirect_uri).await?
        },
    };
    
    // Найти или создать пользователя
    let user = oauth_service.find_or_create_user(&state.db, &oauth_info).await?;
    
    // Создать токены
    let access_token = auth_service.create_access_token(&user)?;
    let refresh_token = auth_service.create_refresh_token(&state.db, user.id, None).await?;
...
pub async fn revoke_session(
    ...
    let result = sqlx::query(
        "UPDATE refresh_tokens SET revoked = 1 WHERE token = $1 AND user_id = $2"
    )
    .bind(&token)
    .bind(user.0.id)
    .execute(&state.db)
