...
- Временная блокировка аккаунта после 60 неудачных попыток
...
```mermaid
graph TD
    A[Пользователь отмечает выполнение] --> B[Проверка статуса]
    B --> C{Уже выполнено?}
...
fn generate_fantasy_quest(text, difficulty, level):
  title = "Священная миссия: {essence}"
...
**Генерация подзадач**:
```rust
difficulty 1: ["Завершить задачу: {text}"]
difficulty 2-5:
  - "Фаза подготовки"
  - "Фаза выполнения 1"
  - "Фаза выполнения 2"
...
+##### 401 Unauthorized
+
+- При истечении срока действия access token возвращается ответ:
+  ```json
+  {
+    "error": "Token expired",
+    "code": "TOKEN_EXPIRED"
+  }
+  ```
+  Дополнительно выставляются заголовки `WWW-Authenticate: Bearer error="invalid_token", error_description="The access token expired"` и `X-Token-Expired: true`. Клиент должен инициировать refresh-процесс через `/api/auth/refresh`.
+- При повреждённом или недействительном токене возвращается:
+  ```json
+  {
+    "error": "Invalid token",
+    "code": "TOKEN_INVALID"
+  }
+  ```
+  Заголовок `WWW-Authenticate` присутствует с описанием ошибки.
+
 ---
...
```json
{
  "title": "Священная миссия: Изучить Kotlin",
...
- Профиль героя с характеристиками D&D
- Статистика и графики
- Kotlin Multiplatform поддержка
- iOS target в shared модуле
- Бумажная карта мира с геолокацией
...
- Удалены все эмоджи из кода
- Добавлены комментарии в код
- Улучшена работа геолокации
- Улучшена работа уведомлений
